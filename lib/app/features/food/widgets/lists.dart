import 'package:suprapp/app/core/constants/app_images.dart';

final List<Map<String, String>> categories = [
  {'image': AppImages.offers, 'text': 'Offers'},
  {'image': AppImages.burger, 'text': 'Burger'},
  {'image': AppImages.healthy, 'text': 'Healthy'},
  {'image': AppImages.asian, 'text': 'Asian'},
  {'image': AppImages.exclusiveOffer, 'text': 'Exclusive Offer'},
  {'image': AppImages.bestSelling, 'text': 'Best Selling'},
  {'image': AppImages.chickenWings, 'text': 'Chicken & Wings'},
  {'image': AppImages.russian, 'text': 'Russian'},
  {'image': AppImages.gotYou, 'text': 'GOT YOU'},
  {'image': AppImages.lebanese, 'text': 'Lebanese'},
  {'image': AppImages.pizza, 'text': 'Pizza'},
  {'image': AppImages.indian, 'text': 'Indian'},
  {'image': AppImages.dessert, 'text': 'Dessert'},
  {'image': AppImages.cofee, 'text': 'Cofee'},
  {'image': AppImages.sushi, 'text': 'Sushi'},
  {'image': AppImages.breakfast, 'text': 'BreakFast'},
];
final List<Map<String, dynamic>> burgerList = [
  {
    'image':
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQtsy5ZBbucwY5XFLqo_wlVUcWwnjqIq3uUhg&s",
    'title': 'Eeetwell',
    // 'rating': 4.5,
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',

    // 'foodItems': 'Fast Food',
    // 'distance': '. 1.7 km',
    // 'arriveTime': ' . 35-45 mins'
  },
  {
    'image':
        "https://media.post.rvohealth.io/wp-content/uploads/2021/07/Starbucks_Food_Spinach_Feta_Wrap-732x549-thumbnail-732x549.jpg",
    'title': 'Yo!-Sushi',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
    // 'foodItems': 'Juices, Sand',
    // 'distance': '. 1.9 km',
    // 'rating': 4.5,
    // 'arriveTime': ' . 35-45 mins'
  },
  {
    'image':
        'https://m-foodz.com/pk/wp-content/uploads/2023/05/Fast-Food-Restaurants-In-Karachi.jpg',
    'title': 'Eeetwell',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
    // 'rating': 4.5,
    // 'foodItems': 'Fast Food',
    // 'distance': '. 1.7 km',
    // 'arriveTime': ' . 35-45 mins'
  },
  {
    'image':
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTPIVQ3-mRyGlCWo38s2i1JFRdaABZqKpbgpA&s",
    'title': 'Yo!-Sushi',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
    // 'foodItems': 'Juices, Sand',
    // 'distance': '. 1.9 km',
    // 'rating': 4.5,
    // 'arriveTime': ' . 35-45 mins'
  },
];
final List<Map<String, dynamic>> chaatCravingList = [
  {
    'image':
        "https://upload.wikimedia.org/wikipedia/commons/thumb/e/e9/Pani_Puri1.JPG/960px-Pani_Puri1.JPG",
    'title': 'Pani Puri',
    // 'rating': 4.5,
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',

    // 'foodItems': 'Fast Food',
    // 'distance': '. 1.7 km',
    // 'arriveTime': ' . 35-45 mins'
  },
  {
    'image':
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSKyyF00sBq-rPoQLt6IIchD2skgKTBxPRUhA&s",
    'title': 'Bhel Puri',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
    // 'foodItems': 'Juices, Sand',
    // 'distance': '. 1.9 km',
    // 'rating': 4.5,
    // 'arriveTime': ' . 35-45 mins'
  },
  {
    'image':
        'https://www.flourandspiceblog.com/wp-content/uploads/2022/04/IMG_7102.jpg',
    'title': 'Channa Chat',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
    // 'rating': 4.5,
    // 'foodItems': 'Fast Food',
    // 'distance': '. 1.7 km',
    // 'arriveTime': ' . 35-45 mins'
  },
  {
    'image':
        "https://www.teaforturmeric.com/wp-content/uploads/2022/04/Dahi-Bhalla-06.jpg",
    'title': 'Dahi Bhallay',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
    // 'foodItems': 'Juices, Sand',
    // 'distance': '. 1.9 km',
    // 'rating': 4.5,
    // 'arriveTime': ' . 35-45 mins'
  },
];
final List<Map<String, dynamic>> cofees = [
  {
    'image':
        "https://www.allrecipes.com/thmb/Hqro0FNdnDEwDjrEoxhMfKdWfOY=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/21667-easy-iced-coffee-ddmfs-4x3-0093-7becf3932bd64ed7b594d46c02d0889f.jpg",
    'title': 'Cold Cofee',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
  },
  {
    'image':
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSvthhy8frIY2WeY4dgkTfGd9zzChCeVpv5zw&s",
    'title': 'Cappucino',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
    // 'foodItems': 'Juices, Sand',
    // 'distance': '. 1.9 km',
    // 'rating': 4.5,
    // 'arriveTime': ' . 35-45 mins'
  },
  {
    'image':
        'https://mocktail.net/wp-content/uploads/2022/03/homemade-Iced-Americano-recipe_1ig.jpg',
    'title': 'Americano',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
    // 'rating': 4.5,
    // 'foodItems': 'Fast Food',
    // 'distance': '. 1.7 km',
    // 'arriveTime': ' . 35-45 mins'
  },
];
final List<Map<String, dynamic>> blisfulbites = [
  {
    'image':
        "https://www.seriouseats.com/thmb/Lkr5DnY7jNP2aP5DS3d5qE0PEkQ=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/__opt__aboutcom__coeus__resources__content_migration__serious_eats__seriouseats.com__2020__08__20200816-nimbu-soda-vicky-wasik-1-28079d5d45ee4e47a37a969d1e4834a0.jpg",
    'title': 'Lemanade',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
  },
  {
    'image':
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR8bq8s2PsInxF5M1t6kMgsbNxhh9vf50y1YA&s",
    'title': 'Fries',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
    // 'foodItems': 'Juices, Sand',
    // 'distance': '. 1.9 km',
    // 'rating': 4.5,
    // 'arriveTime': ' . 35-45 mins'
  },
  {
    'image':
        'https://mocktail.net/wp-content/uploads/2022/03/homemade-Iced-Americano-recipe_1ig.jpg',
    'title': 'Americano',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
    // 'rating': 4.5,
    // 'foodItems': 'Fast Food',
    // 'distance': '. 1.7 km',
    // 'arriveTime': ' . 35-45 mins'
  },
];
final List<Map<String, dynamic>> quickGrabs = [
  {
    'image':
        "https://www.seriouseats.com/thmb/4HxqFS6SHsh1XBCXWWHbsyrGOxI=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/20210909-SAMOSAS-ANDREWJANJIGIAN-86-ca872c2eae8e4e7eb4e7b47cfad8715e.jpg",
    'title': 'Alo Samosa',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
  },
  {
    'image':
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQrFx72RrndllXUh0vh0Ljt-I_jSghw_wLlnA&s",
    'title': 'Qeema Samosa',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
  },
  {
    'image':
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRoO-Sa3oXjmOAq2jptEqJ3RfOhMGKkYaWmNA&s',
    'title': 'Oreo Shake',
    'oldPrice': 279,
    'newPrice': 270,
    'offerText': '10% Off',
    'portion': '1 Portion',
  },
];
final List<Map<String, dynamic>> crazyDelight = [
  {
    'image':
        'https://www.indianveggiedelight.com/wp-content/uploads/2023/07/aloo-tikki-featured-500x375.jpg',
    'title': 'Peri Peri Potato Wedges',
    'subtitle': 'Thick, crispy potato wedges seasoned with zesty peri-peri...',
    'priceBefore': '₹119',
    'priceNow': '₹109',
    'tag': 'BESTSELLER',
    'portion': '1 Portion',
  },
  {
    'image':
        'https://static.toiimg.com/thumb/54659021.cms?imgsize=275086&width=800&height=800',
    'title': 'Fries',
    'subtitle': 'Thick, crispy potato wedges seasoned with zesty peri-peri...',
    'priceBefore': '₹119',
    'priceNow': '₹109',
    'tag': 'BESTSELLER',
    'portion': '1 Portion',
  },
  {
    'image':
        'https://www.ohmyveg.co.uk/wp-content/uploads/2024/10/samosa-chaat-1-1200x1200.jpg',
    'title': 'Samosa Chaat',
    'subtitle': 'Thick, crispy potato wedges seasoned with zesty peri-peri...',
    'priceBefore': '₹119',
    'priceNow': '₹109',
    'tag': 'BESTSELLER',
    'portion': '1 Portion',
  },
];
final List<String> brandsList = [
  'https://logos-world.net/wp-content/uploads/2020/04/McDonalds-Logo.png',
  'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/2a/cf/b9/28/cheezious-pizza.jpg?w=900&h=500&s=1',
  'https://play-lh.googleusercontent.com/qR-sPEGkCQvborNj2jqOF00EV3N9vxJMXYwaufVQMnIspanioOVOqLUSB4kLOkzKYJE7=w600-h300-pc0xffffff-pd',
  'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR4mn74PTlVHE-UElXmyr5dVzzs8X4MDG8pzQ&s'
];
final Map<String, List<Map<String, dynamic>>> categoryProducts = {
  'Burger': [
    {
      'image':
          'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQBFCimCM1wFvb3FZQOZH_j5ta4Qd2SlNj2vg&s',
      'title': 'Cheese Burger',
      'oldPrice': 250,
      'newPrice': 200,
      'offerText': '20% OFF',
      'portion': '1 pc',
    },
    {
      'image':
          'https://images.immediate.co.uk/production/volatile/sites/2/2015/04/2015-02-24-olive-test-d5b505c.jpg?crop=7px%2C1865px%2C3278px%2C1410px&resize=1200%2C630',
      'title': 'Double Beef Burger',
      'oldPrice': 300,
      'newPrice': 260,
      'offerText': '15% OFF',
      'portion': '1 pc',
    },
  ],
  'Pizza': [
    {
      'image':
          'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTedUeWfNA42ORivu3l2whWDkXNY9FiL_IIMA&s',
      'title': 'Pepperoni Pizza',
      'oldPrice': 500,
      'newPrice': 450,
      'offerText': '10% OFF',
      'portion': '8 slices',
    },
    {
      'image':
          'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/2b/b1/83/65/caption.jpg?w=900&h=500&s=1',
      'title': 'Margherita Pizza',
      'oldPrice': 400,
      'newPrice': 350,
      'offerText': '12% OFF',
      'portion': '8 slices',
    },
  ],
  'Healthy': [
    {
      'image':
          'https://www.simplyrecipes.com/thmb/0NrKQlJ691l6L9tZXpL06uOuWis=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/Simply-Recipes-Easy-Greek-Salad-LEAD-2-4601eff771fd4de38f9722e8cafc897a.jpg',
      'title': 'Greek Salad',
      'oldPrice': 180,
      'newPrice': 160,
      'offerText': '11% OFF',
      'portion': '1 bowl',
    },
    {
      'image':
          'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRXkrOboCpn0QOd3oCQ-qea1yJFiyIdIf7Vfw&s',
      'title': 'Green Smoothie',
      'oldPrice': 220,
      'newPrice': 190,
      'offerText': '14% OFF',
      'portion': '350 ml',
    },
  ],
};
